...
* **Per-component proration** (`SalaryComponent.prorateByAttendance`, migration `0007`): earnings prorate by the attendance ratio by default, same as before - a component can opt out (checkbox in Salary > Components) to always pay its full amount regardless of days worked/LOP, for allowances that aren't attendance-linked. The choice is copied into `LineFlags` and stored on the salary snapshot at assignment time, so a later change to the component never rewrites an already-assigned salary or past payroll - matching every other statutory flag. Deductions were never prorated by attendance in the first place (a loan EMI, for instance), so this only changes earnings. Missing on a pre-existing snapshot (Prisma default `true`) behaves exactly as before the flag existed.
